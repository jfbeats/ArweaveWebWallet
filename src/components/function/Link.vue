<template>
	...
	<a v-else-if="to && !attrs.disabled" ref="input" :href="hrefRouter" @click.capture="runFunctions" :class="{ 'router-link-active': isActive, 'router-link-exact-active': isExactActive }"><slot /></a>
	<button v-else-if="runFunctions || attrs.disabled" ref="input" @click.capture="runFunctions" type="button"><slot /></button>
