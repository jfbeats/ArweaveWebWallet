	...
	<span class="icon">
		<template v-for="i in duplicate">
			<transition name="fade-fast" mode="out-in">
				...
				<img v-else v-show="loaded" @load="loaded = true" class="img no-select" :key="key + '-'" :src="iconData" draggable="false" />
			</transition>
		</template>
	</span>
</template>
...
const iconData = computed(() => (isRef(props.icon) ? props.icon.value : props.icon) as string)
...
const duplicate = computed(() => props.glow ? 2 : 1)
watch(iconData, () => loaded.value = false)
const thickness = 5
